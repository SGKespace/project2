...
async def get_active_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    conn = await create_connection()
    await conn[0].execute('SELECT * FROM projects WHERE delivery_by_courier = 1')
    list_ord = await conn[0].fetchall()  # возвращается кортеж

    await close_connection(conn)
    print('The function "get_active_orders" works')

    await update.message.reply_text(text=list_ord, parse_mode="html")
    # await update.callback_query.edit_message_text(text=list_ord)


async def get_overdue_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    print('The function "get_overdue_orders" works')


async def get_ad_report(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    print('The function "get_ad_report" works')
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHOOSING: [MessageHandler(filters.Regex(
                "^(Список активных заказов|Просроченные заказы|"
                "Формирование отчета по рекламе)$"), regular_choice)],
            ...
            GET_ACTIVE_ORDERS: [MessageHandler(filters.TEXT, get_active_orders)],
            GET_OVERDUE_ORDERS: [MessageHandler(filters.TEXT, get_overdue_orders)],
